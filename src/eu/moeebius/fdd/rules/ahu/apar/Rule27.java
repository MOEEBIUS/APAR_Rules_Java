...
/**
 * Implementation of Rule #27 of the APAR rule set: Mixed air temperature should be between return air temperature and outside air temperature (mixed air temperature is too low).
 * @author Georgios D. Kontes
 */
public class Rule27 implements AparInterface {

	private String name = "Rule27";
	...
	/**
	 * Instantiates Rule #27 of the APAR rule set
	 */
	public Rule27() {}
	 ...
	 */
	public Rule27(RealMatrix inputs) {
		this.inputs = inputs;
	}
	 ...
	 */
	public Rule27(RealMatrix inputs, RealMatrix parameters) {
		this.inputs = inputs;
	...
	public double apply() {
		...
		if(this.steadyStateMode > 0){
			for(int ii=0; ii < tma.length; ii++){
				if(tma[ii] > Math.max(tra[ii],  toa[ii]) + et){
	...
	/**
	 * Calculation of Rule #27 of APAR as a static function.
